...
%define haproxy_unitdir %{_unitdir}
...
Source1:        %{name}-%{version}/contrib/systemd/%{name}.service.in
Source2:        %{name}.cfg
...
%{__install}  -p -D -m 0644  contrib/systemd/%{name}.service.in %{buildroot}/%{_unitdir}/%{name}.service
%{__install}  -p -D -m 0755  examples/acl-content-sw.cfg %{buildroot}%{_sysconfdir}/%{name}/%{name}.cfg
%{__install} -c -m 0755 examples/%{name}.init %{buildroot}%{_sysconfdir}/rc.d/init.d/%{name}
%{__install} -p -m 0755 %{name}-systemd-wrapper %{buildroot}/%{_sbindir}/%{name}-systemd-wrapper
%{__install} -p -m 0755 contrib/halog/halog %{buildroot}%{_bindir}/halog
%{__install} -p -m 0755 contrib/iprange/iprange %{buildroot}%{_bindir}/iprange

sed -i "s/^Environment=//g" %{buildroot}/%{_unitdir}/%{name}.service
sed -i 's/"//g' %{buildroot}/%{_unitdir}/%{name}.service
sed -i 's/@SBINDIR@/\/usr\/bin/g' %{buildroot}/%{_unitdir}/%{name}.service
sed -i 's/haproxy-systemd-wrapper/haproxy/g' %{buildroot}/%{_unitdir}/%{name}.service
sed -i "s/\$CONFIG/\/etc\/haproxy\/haproxy.cfg/g" %{buildroot}/%{_unitdir}/%{name}.service
sed -i "s/\$PIDFILE/\/var\/run\/haproxy.pid -Ds/g" %{buildroot}/%{_unitdir}/%{name}.service
...
%changelog
* Tue Jun 21 2016 Ryan O'Hara <rohara@redhat.com> - 1.7.8-3
- Fix TCP user timeout patch for 1.7.8 release

* Thu Jun 16 2016 Ryan O'Hara <rohara@redhat.com> - 1.7.8-2
- Add TARGET to install-bin for haproxy-systemd-wrapper

* Wed Jun 15 2016 Ryan O'Hara <rohara@redhat.com> - 1.7.8-1
- Update to stable release 1.7.8 (#1344012)
